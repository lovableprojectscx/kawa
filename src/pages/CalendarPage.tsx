...
import { CalendarDays, Plus, Clock, X, ChevronLeft, ChevronRight, Trash2 } from "lucide-react";
...
const CalendarPage = () => {
  ...
  const handleAddEvent = async () => {
    ...
    try {
      ...
      const startTime = new Date(currentYear, currentMonth, selectedDay, parseInt(hours), parseInt(minutes));
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        >
          <div className="grid grid-cols-7 gap-1 mb-2">
            {DAYS.map((d) => (
              <div key={d} className="text-center text-[11px] text-muted-foreground font-display tracking-wider uppercase py-2">
                {d}
              </div>
            ))}
          </div>
          ...
          <div className="space-y-3">
            {dayEvents(selectedDay).length > 0 ? (
              dayEvents(selectedDay).map((ev, i) => (
                <div key={i} className="group flex items-start gap-3 px-4 py-3 rounded-md bg-muted/50 border border-border/50">
                  <Clock className="w-4 h-4 text-muted-foreground mt-0.5 shrink-0" />
                  <div className="flex-1">
                    <p className="text-sm text-foreground font-light">{ev.event_title}</p>
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-xs text-muted-foreground">
                        {new Date(ev.start_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false })}
                      </span>
                      <span className={cn("text-[10px] px-2 py-0.5 rounded-full font-display tracking-wider uppercase bg-primary/10 text-primary")}>
                        {ev.type}
                      </span>
                    </div>
                  </div>
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-xl font-display font-semibold text-foreground">
                  Nuevo Evento — {selectedDay} de {viewDate.toLocaleDateString('es-ES', { month: 'long' })}
                </h3>
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-[10px] uppercase tracking-widest text-muted-foreground mb-1.5 block">Título del evento</label>
                  <input
                    type="text"
                    value={newEvent.title}
                    onChange={(e) => setNewEvent({ ...newEvent, title: e.target.value })}
                    placeholder="Ej: Reunión Estratégica"
                    className="w-full bg-muted/50 border border-border rounded-lg px-4 py-2.5 outline-none focus:border-primary/50 transition-all"
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-muted/50 border border-border rounded-lg px-4 py-2.5 outline-none focus:border-primary/50 transition-all appearance-none"
                    >
                  ...
                  <div>
                    <label className="text-[10px] uppercase tracking-widest text-muted-foreground mb-1.5 block">Hora</label>
                    <input
                      type="time"
                      value={newEvent.time}
                ...
                <button
                  ...
                  className="w-full bg-primary text-primary-foreground font-display py-3 rounded-lg hover:bg-primary/90 transition-all mt-4 font-bold tracking-wide"
                >
                  GUARDAR EN KAWA
                </button>
              </div>
